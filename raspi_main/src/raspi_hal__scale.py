...
WEIGHT_TOPIC = "nucleo_scale__weight"
...
class hal__scale(serial_node, measure_node):
    def __init__(self, completion_callback:Callable[[float], None]):
        self.weight_sub = rospy.Subscriber(WEIGHT_TOPIC, Float32, self.weight_update)
        self.weight = -100000
        self.new_timer = lambda : Timer(MEASUREMENT_DURATION, self._completion_callback)
        self.timer = self.new_timer()
        self.completion_callback = completion_callback

        super().__init__(watchdog_sub_topic=WEIGHT_TOPIC, watchdog_sub_type=String)
        super(measure_node, self).__init__()
    ...
    def complete(self) -> bool: # intialized and node online and measurement complete
        return self.weight != -100000 and self.online() and not self.timer.is_alive()
